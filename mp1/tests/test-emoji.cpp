...
TEST_CASE("`emoji_count` returns zero with no emojis", "[weight=3][part=1]") {
  char *s = (char *) malloc(100);
...
TEST_CASE("`emoji_invertChar` inverts smiley face into another emoji", "[weight=3][part=1]") {
  char *s = (char *)malloc(100);
...
TEST_CASE("`emoji_invertChar` does not invert string with no emoji", "[weight=3][part=1]") {
  char *s = (char *) malloc(100);
...
TEST_CASE("`emoji_random_alloc` allocates new memory", "[weight=3][part=1]") {
...
TEST_CASE("`emoji_invertFile_alloc` inverts all smiley faces - short text", "[weight=3][part=1]") {
...
TEST_CASE("`emoji_invertFile_alloc` inverts all smiley faces - long text", "[weight=3][part=1]") {
  ...
  testing_emoji[4] = '\0';
  REQUIRE(strcmp(testing_emoji, "\xF0\x9F\x98\x8A") != 0);
  printf("s%", inverted_content);
  REQUIRE(strlen((char *) inverted_content) == 19);
  free(testing_emoji);
  free(inverted_content);
}

TEST_CASE("`emoji_count` counts no emoji", "[weight=3][part=1]") {
  int count_emoji_content = emoji_count("tests/txt/noemoji.txt");
  REQUIRE(count_emoji_content == 0);
}

TEST_CASE("`emoji_invertFile_alloc` inverts all emojis - no emoji in text", "[weight=3][part=1]") {
  unsigned char *inverted_content = emoji_invertFile_alloc("tests/txt/noemoji.txt");
  REQUIRE(inverted_content != NULL );
  int count_emoji_content = emoji_count("tests/txt/noemoji.txt");
  REQUIRE(count_emoji_content == 0);
  free(inverted_content);
...
TEST_CASE("`emoji_invertFile_alloc` invalid file name", "[weight=3][part=1]") {
